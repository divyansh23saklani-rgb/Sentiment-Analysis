...
def analyze_sentiment(text):
    ...
    if importance >= 0.8:
        recommendations.append("Monitor for signs of distress related to this memory")
        recommendations.append("Consider reminiscence therapy with positive memories of mentioned family members")
        recommendations.append("Consider journaling activity about positive memories")
    
    if grief_indicators:
        recommendations.append("Validate feelings and provide emotional support")
        recommendations.append("Consider scheduled discussion of memories in controlled therapeutic setting")
    
    if combined_score <= -0.3:  # Strongly negative sentiment
        recommendations.append("Consider distraction techniques if distress increases")
        recommendations.append("Schedule enjoyable activity following difficult memory discussions")
        recommendations.append("Monitor for signs of depression or withdrawal")

    # Format results for both printing and JSON
    results = {
        "meta": {
        ...
        "sentiment_analysis": {
            ...
            "distilbert": {
                ...
                "score": round(distilbert_score, 4),
                "mapped_score": round(distilbert_compound, 4)
            }
        },
        ...
        "temporal_context": {
            "event_age": event_age,
            "event_date": raw_date,
            "time_indicators": time_of_day
        },
        "spatial_context": {
            "location": event_location
        },
        "activity_references": detected_adls if detected_adls else [],
        "relationship_indicators": {
            "family_mentions": family_mentions if family_mentions else []
        },
        "emotional_indicators": {
            "grief_indicators": grief_indicators if grief_indicators else [],
            "trauma_indicators": trauma_indicators if trauma_indicators else [],
            "severity_indicators": severity_indicators
        },
        "care_needs": {
            "potential_concerns": potential_concerns if potential_concerns else [],
    ...
    print("\n✅ Data saved to structured_memories.json")
    
    return results

def save_to_json(results, json_path="structured_memories.json"):
    ...
    if os.path.isfile(json_path) and os.path.getsize(json_path) > 0:
        try:
            ...
            # If the file exists but is not valid JSON, create new list
            data = []
    else:
        # Create new list if file doesn't exist
        data = []
    
    # Append new results
    data.append(results)
    
    # Write data back to file with pretty formatting
    with open(json_path, 'w', encoding='utf-8') as jsonfile:
        json.dump(data, jsonfile, indent=2, ensure_ascii=False)

if __name__ == "__main__":
    print("\n💬 Alzheimer's Patient Monitoring - Sentiment & Memory Analysis\n")
    print("📊 Results will be saved to structured_memories.json for narration system")
    while True:
        try:
